...
public class MartRepositoryTest {
        ...
        // 주문
        @Test
        public void orderDeliveryInsertTest() {
                // 멤버 정보
                Member member = Member.builder().id(1L).build();
                // 아이템 정보
                // Item item = Item.builder().id(2L).price(10000).build();
                Item item = itemRepository.findById(2L).get();

                // 배송지 입력
                Delivery delivery = Delivery.builder()
                                .city("서울시").street("123-12").zipcode("11160")
                                .deliveryStauts(DeliveryStatus.READY).build();
